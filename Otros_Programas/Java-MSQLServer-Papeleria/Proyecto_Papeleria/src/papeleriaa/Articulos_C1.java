...
public class Articulos_C1 extends JFrame implements ActionListener{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Articulos_C1 frame = new Articulos_C1();
	...
	public Articulos_C1() {
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"Clave", "Tipo", "Descripción", "Cantidad", "$Compra", "$Venta", "proveedor"
			}
		));
		...
		ResultSet rs1 = st1.executeQuery("Select * from articulos");
		
		while(rs1.next()) {
			int cve = rs1.getInt(1);
			String tip = rs1.getString(2);
			String des = rs1.getString(3);
			int can= rs1.getInt(4);
			int pc= rs1.getInt(5);
			int pv= rs1.getInt(6);
			String prov= rs1.getString(7);
	
			
	        Object datos1[] = {cve, tip, des, can, pc, pv, prov};
...
//	        Object datos1[] = {nom, ape, ocupa, sueldo};
	...
	public void llamada_busca(String nombre, String busca) {
		...
		if(nombre == "cve") {
			 rs1 = st1.executeQuery("Select * from articulos where cve = "+Integer.parseInt(busca));
		}
		else {
			rs1 = st1.executeQuery("Select * from articulos where "+nombre+" like '%"+busca+"%'");
