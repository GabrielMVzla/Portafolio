...
//		for(int i=0; i<every_token.size();i++) { System.out.println(every_token.get(i));	}
	...
	public void analizadorLexico(String token) 
	{
		...
		if(token.matches("^[0-9][0-9][0-9]+?$")) {//error en numeros
			resultado.add("Se esperaba una longitud de 2 dígitos en el número \"" + token +"\"");
		}
		...
		if(tipo==-1) {
			...
			if(mat.find())
				...
				resultado.add("Error Léxico en la linea \""+renglon+"\" No. de token \""+columna+"\" nombre del token \""+token+"\", los identificadores deben llevar al menos un número al final");
